...
import { modifyStatSchedule } from "../../api/StatAPI";
...
const StatDescription = () => {
  const {params:{id,createdAt,updatedAt,name,scheduleType,
    timeOfDay,timeTaken,startDate,dueDate,statTypeName,streak,totalTimes,totalTimeSpent,
  ...
  const onUpdateScheduleType = async() =>{
    ...
    if (isScheduleEditing){
      await modifyStatSchedule(config, 'Bearer '+user.accessToken,
