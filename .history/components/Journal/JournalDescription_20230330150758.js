...
import { modifyJournalSchedule } from "../../api/JournalAPI";
...
const JournalDescription = () => {
  const {params:{id,createdAt,updatedAt,name,scheduleType,
    timeOfDay,timeTaken,startDate,dueDate,journalTypeName,streak,totalTimes,totalTimeSpent,
  ...
  const onUpdateScheduleType = async() =>{
    ...
    if (isScheduleEditing){
      await modifyJournalSchedule(config, 'Bearer '+user.accessToken,
