...
const AddHabitForm = (props) => {
  ...
  return (
    <View className="py-3 px-2 bg-gray-600">
      ...
      <View>
        {showStartDate?
         <DatePicker selected={getFormatedDate(new Date(startDate),'YYYY/MM/DD')} 
         onSelectedChange={date=>modifyStartDate(date)} mode="calendar"/>
         :null}
         {showDueDate?
         <DatePicker selected={getFormatedDate(new Date(dueDate),'YYYY/MM/DD')}
