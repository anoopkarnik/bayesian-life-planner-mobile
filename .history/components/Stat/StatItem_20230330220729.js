...
const StatItem = (props) => {
	...
	const onComplete = async() => {
		await completeStat(config,'Bearer '+user.accessToken,props.record.id,user.id)
		await props.refreshFunction(config,'Bearer '+ user.accessToken,props.record.statTypeName,showActive)
	}

	const onShowDescription = async() =>{

		navigation.navigate("Description",{id,createdAt,updatedAt,name,scheduleType,
        timeOfDay,timeTaken,startDate,dueDate,statTypeName,streak,totalTimes,
    ...
    <View className="flex">
        <View className="flex flex-row flex-1">
            ...
            <View className="w-1/5 mx-2 my-2">
              <Text className="text-xl">{props.record.value}</Text>
            </View>
