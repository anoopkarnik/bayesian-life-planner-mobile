...
import ChildJournalItem from './ChildJournalItem';
import AddChildJournalForm from './AddChildJournalForm';
// import AddChildJournalForm from './AddChildJournalForm';
...
const JournalItem = (props) => {
	...
	const [showChildJournals, setShowChildJournals] = useState(false);
    ...
    <View className="flex">
        <View className="flex flex-row flex-1">
            <View className="w-3/5 flex-row">
                {props.record.journalResponses.length>0?<>
                {showChildJournals?
                    <MinusCircleIcon color="black" height={30} width={20}
                    onPress={()=>setShowChildJournals(false)}/>:
                    <PlusCircleIcon color="black" height={30} width={20}
                    onPress={()=>setShowChildJournals(true)}/>
                }</>:null}
        ...
        {showChildJournals?
          <View>
            {props.record.journalResponses.map((record)=>(
                <ChildJournalItem key={record.id} record={record} refreshFunction={props.refreshFunction}/>
            ))}
          </View>:null}
        {showAddJournal?
          <AddChildJournalForm refreshFunction={props.refreshFunction} 
          name={props.record.name} type={props.record.journalTypeName}/>:null}
